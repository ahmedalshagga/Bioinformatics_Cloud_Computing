...
#Loading our results
our_data <- read.table(group_results_path, header = TRUE, stringsAsFactors = FALSE)

group_results <- our_data %>%
  filter(status == "OK", sample_1 == "WT_Iso", sample_2 == "gln3_Iso") %>%
...
#Read the paper table WITHOUT header; keep all rows
...
#Drop the first 3 header rows and build a group_results-like df
#WT (1.3% Isobutanol) / WT (0%) is in columns V30–V32
paper_dat <- paper_raw %>%
  dplyr::slice(4:n()) %>%
dplyr::select(-c(17, 21, 25, 29, 33))# rows 4:end = actual data
# Manually assign column names based on the file structure
...
print(paste("Loaded", nrow(paper_raw), "genes from papers analysis."))
...
While the general biological trends were successfully reproduced, we observed a significant reduction in our total gene count compared to the original study. (3326 vs 6535 gene count) This reduction arises primarily from differences in genome annotation databases: we utilized the conservative Ensembl GTF (\~6,600 verified protein-coding genes), whereas the authors employed the comprehensive SGD annotation (\~13,000 features), leading to this reduction. The application of Cuffdiff in our pipeline reduced this count to \~3,300 genes for statistical testing.

Due to depreciation of the TopHat aligner with the oldest available version being 2.1.0 (compared to the papers 2.0.9), exact replication of the original alignment was not possible.

We also generated this count via explicit filtering of rRNA and tRNA features of the Ensembl GTF, as the authors did not specify their filtering critiera or feature types used leading to difficulty in exact reproduction.

Nonetheless, despite these technical differences, our results show a very close correlation with the results published in the paper and we were able to reproduce the key biological discoveries reported in the paper.
...
# Expand comma-separated gene lists and assign category
...
# Add categories via gene2cat_sys (SystematicName -> category) if available
our_5b <- merge(
  our_5b,
  gene2cat_sys,
  by = "gene_id",
...
# Build our 5A: WT_NoIso vs WT_Iso 
our_5a <- our_data %>%
  dplyr::filter(
    status   == "OK",
    sample_1 == "WT_NoIso",   # WT without isobutanol
    sample_2 == "WT_Iso"      # WT with isobutanol
  ) %>%
  dplyr::transmute(
    gene_id,
    locus,
...
#Build paper 5B: Gln3Iso / WTIso from paper_dat
...
# Save our Figure 5A
ggsave(
  filename = file.path(plots_dir, "Figure_5A_our_pipeline.png"),
  plot     = fig5a_ours,
  width    = 6,
  height   = 6,
  dpi      = 300
...
# Save our Figure 5B
ggsave(
  filename = file.path(plots_dir, "Figure_5B_our_pipeline.png"),
  plot     = fig5b_ours,
  width    = 6,
  height   = 6,
  dpi      = 300
